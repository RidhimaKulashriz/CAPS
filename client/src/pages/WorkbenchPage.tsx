...
import { Activity, ArrowDown, ArrowUpRight, BookOpen, Box, BrainCircuit, CheckCircle2, ChevronRight, CircleDot, Code2, Database, Download, FileText, Gauge, GitBranch, Grid3X3, Layers3, Maximize2, Microscope, Network, Pause, Play, Radio, RefreshCw, ScanSearch, Search, Server, Settings2, ShieldCheck, SlidersHorizontal, Sparkles, Terminal, Timer, Upload, Wifi, X, ZoomIn, ZoomOut } from "lucide-react";
import { Link } from "wouter";
import { toast } from "sonner";
import { trpc } from "@/lib/trpc";

type Focus = "command" | "cases" | "model" | "prototypes" | "embeddings" | "capsule" | "network" | "observability" | "research" | "videos" | "images" | "papers" | "finder" | "pmc";
...
const focusLabels: Record<Focus, string> = { command: "COMMAND CENTER", cases: "CASE WORKSPACE", model: "MODEL CENTER", prototypes: "PROTOTYPE LAB", embeddings: "EMBEDDING LAB", capsule: "EVIDENCE CAPSULE", network: "NETWORK LAB", observability: "OBSERVABILITY", research: "RESEARCH ROADMAP", videos: "ML VIDEO OBSERVATORY", images: "SCIENTIFIC VISUALIZATION", papers: "RESEARCH PAPERS", finder: "RESEARCH FINDER", pmc: "PMC RESEARCH LIBRARY" };
...
const moreResearchVideos = [
  ...
  },
  {
    ...
    kind: "Breast cancer · DCE-MRI · reconstruction comparison",
  },
  {
    ...
    kind: "Breast cancer · DCE-MRI · ResNet",
  },
  {
    ...
    kind: "Gastric cancer · computer vision · intraoperative AI",
  },
  {
  ...
  },
  {
    ...
    kind: "Breast cancer cells · StarDist/TrackNet",
  },
  {
    ...
    kind: "Breast cancer cells · automated trajectories",
  },
];
...
function FinderPanel({ query, setQuery }: { query: string; setQuery: (v: string) => void }) {
  const records = [...paperRecords.map(p => ({ kind: "PAPER", title: p.title, meta: `${p.authors} · ${p.year}`, href: "/papers", action: "OPEN SHELF" })), ...["Artificial intelligence / visual primer", "Neural network activation field", "Network morphology / microscopy"].map((title, i) => ({ kind: "VIDEO", title, meta: "PMC supplementary MP4 · direct playback", href: "/ml", action: "OPEN VIDEO" })), ...["Whole-slide pathology", "ML research laboratory", "Compute infrastructure", "Tissue morphology atlas"].map(title => ({ kind: "IMAGE", title, meta: "Bundled JPG · local asset", href: "/atlas", action: "OPEN ATLAS" }))];
